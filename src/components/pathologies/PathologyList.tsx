...
import { supabase, type Pathology } from "../../lib/supabase";
...
const PathologyList: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("pathologies").insert([formData]);
      ...
      setError(
        err instanceof Error ? err.message : "Error al guardar patología"
      );
    }
  };
    ...
    <div>
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-2xl font-semibold text-gray-900">Patologías</h1>
          <p className="mt-2 text-sm text-gray-700">
            Gestión de patologías médicas disponibles en el sistema.
          </p>
        </div>
        <div className="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
          ...
          >
            ...
            {showForm ? "Cancelar" : "Nueva Patología"}
          </button>
        </div>
      </div>
